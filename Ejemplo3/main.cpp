...
{	
	...
	m_applicationName = (LPCSTR)"ProyectoGraficasComputacionalesI";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT umessage, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case VK_F1:
		{
			CameraClass* Camara = CameraClass::getCamera();
			float x = Camara->GetXPos();
			float y = Camara->GetYPos() - 7;
			float z = Camara->GetZPos();
			string info = "X: " + to_string(x) + "Y: " + to_string(y) + " z: " + to_string(z);

...
#define _modelo
#ifdef min
#undef min // Undefine min si está definido previamente
#endif

#ifdef max
#undef max // Undefine max si está definido previamente
#endif
#include "Imagenes.h"
#include "VectorRR.h"
...
public:
	...
	// casa bbox custom
	// Bounding box de la pared frontal (Front)
	float m_bbox1[6] = { 711, 914.399536, -4.47733116, 91.2078705, 5.7050018, 9.4050018 };
	// Bounding box de la pared frontal (Front)
	float m_bbox1a[6] = { 672, 688.399536, -4.47733116, 91.2078705, 5.7050018, 9.4050018 };

	// Bounding box de la pared trasera (Back)
	float m_bbox2[6] = { 658.351318, 878.399536, -4.47733116, 91.2078705, 130.856400, 135.856400 };
	// Bounding box de la pared trasera (Back)
	float m_bbox2a[6] = { 898.351318, 908.399536, -4.47733116, 91.2078705, 130.856400, 135.856400 };

	// Bounding box de la pared izquierda (Left)
	float m_bbox3[6] = { 672.351318, 677.351318, -4.47733116, 91.2078705, 8.7050018, 135.856400 };

	// Bounding box de la pared derecha (Right)
	float m_bbox4[6] = { 908.399536, 913.399536, -4.47733116, 91.2078705, 8.7050018, 135.856400 };
	// Bounding box de la pared derecha (mid)
	float m_bbox5[6] = { 790.399536, 800.399536, -4.47733116, 91.2078705, 8.7050018, 135.856400 };
	// Bounding box de la caja frontal
	float m_bbox6[6] = { 828.399536, 870.399536, -4.47733116, 91.2078705, -18.7050018, 5.856400 };
	// Bounding box de la caja lateral
	float m_bbox7[6] = { 658.399536, 672.399536, -4.47733116, 91.2078705, 84.7050018, 135.856400 };
	...
	Modelos(HWND hWnd, OpenGLClass* OpenGL, const char nombreFile[], const wchar_t textura[], const wchar_t normMap[], const wchar_t specMap[], float escala, float x, float y, float z, float numModelo)
	{
		...
		this->SetBBox(box.minPoint.X, box.minPoint.Y, box.minPoint.Z, box.maxPoint.X, box.maxPoint.Y, box.maxPoint.Z);
	}
	...
	void SetBBox(float x, float y, float z, float xmax, float ymax, float zmax)
	{
		this->m_bbox[0] = x + this->x;
		this->m_bbox[1] = xmax + this->x;
		this->m_bbox[2] = y + this->y;
		this->m_bbox[3] = ymax + this->y;
		this->m_bbox[4] = z + this->z;
		this->m_bbox[5] = zmax + this->z;
		return;
	}
	void SetCons()
	{
	...
	BoundingBox GetBoundingBox() const {
		BoundingBox bbox;
		bbox.minPoint = VectorRR(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
		bbox.maxPoint = VectorRR(std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest());

		for (int i = 0; i < cantv; i++) {
			const Vertices& vertex = modelo.maya[i];

			// Actualizar los valores mínimos y máximos en cada eje
			bbox.minPoint.X = std::min(bbox.minPoint.X, vertex.Posx);
			bbox.minPoint.Y = std::min(bbox.minPoint.Y, vertex.Posy);
			bbox.minPoint.Z = std::min(bbox.minPoint.Z, vertex.Posz);
			bbox.maxPoint.X = std::max(bbox.maxPoint.X, vertex.Posx);
			bbox.maxPoint.Y = std::max(bbox.maxPoint.Y, vertex.Posy);
			bbox.maxPoint.Z = std::max(bbox.maxPoint.Z, vertex.Posz);
		}
	...
	void coliply(float xmax, float xmin, float ymax, float ymin, float zmax, float zmin)
	{
		if ((this->m_bbox[0] <= xmax && this->m_bbox[1] >= xmin) && (this->m_bbox[2] <= ymax && this->m_bbox[3] >= ymin) && (this->m_bbox[4] <= zmax && this->m_bbox[5] >= zmin))
		{
			this->iscolply = 1;
	...
	void coliplyc(float xmax, float xmin, float ymax, float ymin, float zmax, float zmin)
	{
		if ((this->m_bbox1[0] <= xmax && this->m_bbox1[1] >= xmin) && (this->m_bbox1[2] <= ymax && this->m_bbox1[3] >= ymin) && (this->m_bbox1[4] <= zmax && this->m_bbox1[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		else setcoliply(false);
		if ((this->m_bbox1a[0] <= xmax && this->m_bbox1a[1] >= xmin) && (this->m_bbox1a[2] <= ymax && this->m_bbox1a[3] >= ymin) && (this->m_bbox1a[4] <= zmax && this->m_bbox1a[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		...
		if ((this->m_bbox2[0] <= xmax && this->m_bbox2[1] >= xmin) && (this->m_bbox2[2] <= ymax && this->m_bbox2[3] >= ymin) && (this->m_bbox2[4] <= zmax && this->m_bbox2[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		if ((this->m_bbox2a[0] <= xmax && this->m_bbox2a[1] >= xmin) && (this->m_bbox2a[2] <= ymax && this->m_bbox2a[3] >= ymin) && (this->m_bbox2a[4] <= zmax && this->m_bbox2a[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		else setcoliply(false);
		if ((this->m_bbox3[0] <= xmax && this->m_bbox3[1] >= xmin) && (this->m_bbox3[2] <= ymax && this->m_bbox3[3] >= ymin) && (this->m_bbox3[4] <= zmax && this->m_bbox3[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		else setcoliply(false);
		if ((this->m_bbox4[0] <= xmax && this->m_bbox4[1] >= xmin) && (this->m_bbox4[2] <= ymax && this->m_bbox4[3] >= ymin) && (this->m_bbox4[4] <= zmax && this->m_bbox4[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		else setcoliply(false);
		if ((this->m_bbox5[0] <= xmax && this->m_bbox5[1] >= xmin) && (this->m_bbox5[2] <= ymax && this->m_bbox5[3] >= ymin) && (this->m_bbox5[4] <= zmax && this->m_bbox5[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		else setcoliply(false);
		if ((this->m_bbox6[0] <= xmax && this->m_bbox6[1] >= xmin) && (this->m_bbox6[2] <= ymax && this->m_bbox6[3] >= ymin) && (this->m_bbox6[4] <= zmax && this->m_bbox6[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}
		else setcoliply(false);
		if ((this->m_bbox7[0] <= xmax && this->m_bbox7[1] >= xmin) && (this->m_bbox7[2] <= ymax && this->m_bbox7[3] >= ymin) && (this->m_bbox7[4] <= zmax && this->m_bbox7[5] >= zmin))
		{
			this->iscolply = 1;
			return;
		}

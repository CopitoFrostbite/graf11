...
public:
	...
	Billboard(HWND hWnd, OpenGLClass* OpenGL, WCHAR textura[], WCHAR NormalBill[], WCHAR SpecularBill[], float escala, float x, float z, int numtext, int numtext2, int numtext3)
	{
		this->escala = escala;
		this->x = x;
		this->y = y;
		...
		EnvioTexturasBill(OpenGL, textura, numtext);
		EnvioTexturasBill(OpenGL, NormalBill, numtext2);
		EnvioTexturasBill(OpenGL, SpecularBill, numtext3);
	}

	Billboard(HWND hWnd, OpenGLClass* OpenGL, WCHAR textura[], WCHAR NormalBill[], WCHAR SpecularBill[], WCHAR textura2[], WCHAR NormalBill2[], WCHAR SpecularBill2[], float escala, float x, float z, int numtext, int numtext2, int numtext3,int numtext4, int numtext5, int numtext6)
	{
		this->escala = escala;
		this->x = x;
		this->y = y;
		...
		EnvioTexturasBill(OpenGL, NormalBill, numtext2);
		EnvioTexturasBill(OpenGL, SpecularBill, numtext3);
		EnvioTexturasBill(OpenGL, textura2, numtext4);
		EnvioTexturasBill(OpenGL, NormalBill2, numtext5);
		EnvioTexturasBill(OpenGL, SpecularBill2, numtext6);
	}

	void EnvioTexturasBill(OpenGLClass* OpenGL, const wchar_t* textura, int numtext) {
		Carga(textura);
	...
	void Render2(OpenGLClass* OpenGL)
	{
		//habilitamos el culling para reducir tiempo de procesamiento de las texturas
		//las texturas se analizan pixel a pixel para determinar que se imprimen o no
		//por lo que aunque no la veamos nos costo, por eso la eliminamos.	
		RenderBuffers2(OpenGL);
		return;
	}

	void RenderBuffers2(OpenGLClass* OpenGL)
	{
	...
	float anguloBill(float xc, float zc)
	{
		float vecx = xc - x;
		float vecz = zc - z;

...
public:
	...
	Terreno(HWND hWnd, OpenGLClass* OpenGL, 
		const wchar_t* alturas, 
		const wchar_t* texturaForest, int numtextFor,
		const wchar_t* texturaForestNorm, int numtextForestNorm,
		const wchar_t* texturaDirt, int numtextDirt,
		const wchar_t* texturaDirtNorm, int numtextDirtNorm,
		const wchar_t* texturaCliff, int numtextCliff,
		const wchar_t* texturaCliffNorm, int numtextCliffNorm,
		const wchar_t* texturaPiedras, int numtextPiedra,
		const wchar_t* texturaPiedrasNormal, int numtextPiedraNorm,
		const wchar_t* texturablend, int numblend,
		float ancho, float prof)
	{
		...
		//bosque
		EnvioText(OpenGL, texturaForest, numtextFor);
		EnvioText(OpenGL, texturaForestNorm, numtextForestNorm);

		//tierra
		EnvioText(OpenGL, texturaDirt, numtextDirt);
		EnvioText(OpenGL, texturaDirtNorm, numtextDirtNorm);

		//peñasco
		EnvioText(OpenGL, texturaCliff, numtextCliff);
		EnvioText(OpenGL, texturaCliffNorm, numtextCliffNorm);

		//Piedras
		EnvioText(OpenGL, texturaPiedras, numtextPiedra);
		EnvioText(OpenGL, texturaPiedrasNormal, numtextPiedraNorm);

		//Blend
		EnvioText(OpenGL, texturablend, numblend);		
	}

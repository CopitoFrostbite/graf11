...
CameraClass* CameraClass::instancia = 0;

CameraClass* CameraClass::getCamera()
{
	if (instancia == 0)
	{
		instancia = new CameraClass();
	}
	return instancia;
}
...
void CameraClass::SetBBox(float x, float y, float z)
{
	this->m_bbox[0] = (x - 1);
	this->m_bbox[1] = (x + 1);
	this->m_bbox[2] = (y+ - 6);
